...
```{r}
SIA_full$Location <- ifelse(SIA_full$Month_Year == "02-25", "EP", "RB")
...
ggplot() +
  ...
  geom_point(data = SIA_full, aes(x = Longitude, y = Latitude, color = Location),
             size = 2) +
  coord_sf(xlim = c(geo.box["xmin"], geo.box["xmax"]),
           ylim = c(geo.box["ymin"], geo.box["ymax"])) +
  theme_minimal() +
...
## Explore spatial differences in isotopes?? particularly entrance point
