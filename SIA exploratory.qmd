...
blood_labs <- lab_outputs %>%
  ...
  left_join(metadata %>% select(Blood_RB, Sex, Life_stage, Date, Mass, Adipose_perc,Turtle_ID),
            by = c("Sample_ID" = "Blood_RB")) %>%
  mutate(Sample_ID = as.character(Sample_ID))

skin_labs <- lab_outputs %>%
  filter(Sample_type == "Skin") %>%
  left_join(metadata %>% select(Biopsy_no, Genetics_no, Sex, Life_stage, Date, Mass,Adipose_perc,Turtle_ID) %>%
